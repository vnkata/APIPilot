...
class ConstraintAssembler:
    ...
    def _build_detail_section(
        ...
        def get_param_description(param_name: str) -> str:
            ...
            if param_name in param_descriptions_cache:
                return param_descriptions_cache[param_name]

            # Get parameter properties
            if operation.parameters and param_name in operation.parameters:
                param = operation.parameters[param_name]
                param_info = param.to_human_readable()
            else:
                # Fallback if parameter not found (defensive)
                logger.debug(
                    f"Parameter '{param_name}' not found in operation {operation_id}, "
                    f"using name only"
                )
                param_info = param_name

            param_descriptions_cache[param_name] = param_info
            return param_info
