...
class ResPropSingleStructuralExtractor:
    ...
    def _extract_array(
        ...
        if item_props.min_items is not None or item_props.max_items is not None:
            ...
            if item_props.min_items is not None:
                args["min"] = item_props.min_items
                evidence_parts.append(f"minItems: {item_props.min_items}")
            if item_props.max_items is not None:
                args["max"] = item_props.max_items
                evidence_parts.append(f"maxItems: {item_props.max_items}")
    ...
    @staticmethod
    def can_extract(item_props: ItemProperties) -> bool:
