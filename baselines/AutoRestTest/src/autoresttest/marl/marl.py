...
class QLearning:
    ...
    def execute_operations(self):
        ...
        # Initialize valid paths list for logging
        valid_paths = []
        all_operations = []
        ...
        while time.time() - start_time < self.time_duration:
            ...
            all_operations.append(operation_id)

            self.tui_output(start_time, operation_id)

            # Log operation sequence path
            logging.info(f"Executing operation: {operation_id}")
            ...
            if data_source == "LLM":
                ...
                # Log dependency sequence path
                dep_ops = set()
                if parameter_dependencies:
                    dep_ops.update(dep["dependent_operation"] for dep in parameter_dependencies.values())
                if request_body_dependencies:
                    dep_ops.update(dep["dependent_operation"] for dep in request_body_dependencies.values())
                if dep_ops:
                    logging.info(f"Dependency sequence path for {operation_id}: depends on {list(dep_ops)}, type: {dependency_type}")
            ...
            # Log valid paths when operation is successful
            if good_response_reward > 0:
                valid_paths.append(operation_id)
                logging.info(f"Successful operation: {operation_id}. Valid path so far: {valid_paths}. Full sequence: {all_operations}")
